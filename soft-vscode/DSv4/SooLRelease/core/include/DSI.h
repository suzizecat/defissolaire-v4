...
//Instances for peripheral DSI
		...
		inline void DSI::enable_clock() volatile
		        {
		            get_clock_enable_reg(get_addr()) |= get_clock_enable_bit(get_addr());
		        }
		
		inline constexpr volatile Reg32_t &DSI::get_clock_enable_reg(const uintptr_t addr) volatile
		{
		switch (addr)
		{
		//Output for DSIEN
		...
		inline constexpr uint32_t DSI::get_clock_enable_bit(const uintptr_t addr) volatile
		{
		switch (addr)
		{
		//Output for DSIEN
